...
def main():
  ...
  vst3_bundle = build_dir + "\\SpectralFiltre.vst3"
  ...
  if not zip:
    installer = "\\build-win\\installer\\SpectralFiltre Installer.exe"

    if demo:
      installer = "\\build-win\\installer\\SpectralFiltre Demo Installer.exe"

    files = [
      projectpath + installer,
      projectpath + "\\installer\\changelog.txt",
      projectpath + "\\installer\\known-issues.txt",
      projectpath + "\\build-win\\manual\\SpectralFiltre manual.pdf"
    ]
    ...
    vst3_bundle = projectpath + "\\build-win\\SpectralFiltre.vst3"
    if os.path.exists(vst3_bundle):
      add_folder_to_zip(zf, vst3_bundle, "SpectralFiltre.vst3")

    # Add standalone executables
    files = [
      projectpath + "\\build-win\\SpectralFiltre_x64.exe",
      projectpath + "\\build-win\\SpectralFiltre_ARM64EC.exe",
    ]
    ...
    clap_files = [
      # Postbuild locations
      (projectpath + "\\build-win\\SpectralFiltre_x64.clap", "SpectralFiltre_x64.clap"),
      (projectpath + "\\build-win\\SpectralFiltre_ARM64EC.clap", "SpectralFiltre_ARM64EC.clap"),
      # Build output locations (fallback)
      (projectpath + "\\build-win\\clap\\x64\\Release\\SpectralFiltre.clap", "SpectralFiltre_x64.clap"),
      (projectpath + "\\build-win\\clap\\ARM64EC\\Release\\SpectralFiltre.clap", "SpectralFiltre_ARM64EC.clap"),
    ]
  ...
  files = [
    projectpath + "\\build-win\\pdbs\\SpectralFiltre-vst3_x64.pdb",
    projectpath + "\\build-win\\pdbs\\SpectralFiltre-vst3_ARM64EC.pdb",
    projectpath + "\\build-win\\pdbs\\SpectralFiltre-app_x64.pdb",
    projectpath + "\\build-win\\pdbs\\SpectralFiltre-app_ARM64EC.pdb",
    projectpath + "\\build-win\\pdbs\\SpectralFiltre-clap_x64.pdb",
    projectpath + "\\build-win\\pdbs\\SpectralFiltre-clap_ARM64EC.pdb"
  ]
