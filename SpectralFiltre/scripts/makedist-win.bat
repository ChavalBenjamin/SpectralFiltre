...
if %DEMO_ARG% == "demo" (
  echo Making SpectralFiltre Windows DEMO VERSION distribution ...
  set DEMO=1
) else (
  echo Making SpectralFiltre Windows FULL VERSION distribution ...
  set DEMO=0
)
...
REM - msbuild SpectralFiltre-app.vcxproj /p:configuration=release /p:platform=x64

echo Building x64 binaries...
REM add projects with /t to build VST2 and AAX
msbuild SpectralFiltre.sln /t:SpectralFiltre-app;SpectralFiltre-vst3;SpectralFiltre-clap /p:configuration=release /p:platform=x64 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly
if %ERRORLEVEL% neq 0 (
...
msbuild SpectralFiltre.sln /t:SpectralFiltre-app;SpectralFiltre-vst3;SpectralFiltre-clap /p:configuration=release /p:platform=ARM64EC /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win-arm64ec.log;errorsonly
if %ERRORLEVEL% neq 0 (
...
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\SpectralFiltre.aaxplugin\Contents\Win32\SpectralFiltre.aaxplugin --out .\build-win\aax\bin\SpectralFiltre.aaxplugin\Contents\Win32\SpectralFiltre.aaxplugin
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\SpectralFiltre.aaxplugin\Contents\x64\SpectralFiltre.aaxplugin --out .\build-win\aax\bin\SpectralFiltre.aaxplugin\Contents\x64\SpectralFiltre.aaxplugin
...
echo Making Installer ...
  ...
  REM :32-Bit-is
  REM REM "%ProgramFiles%\Inno Setup 6\iscc" /Q ".\installer\SpectralFiltre.iss"
  REM goto END-is

  REM :64-Bit-is
  "%ProgramFiles(x86)%\Inno Setup 6\iscc" /Q ".\installer\SpectralFiltre.iss"
  if %ERRORLEVEL% neq 0 (
  ...
  REM -"C:\Program Files (x86)\Microsoft SDKs\Windows\v7.1A\Bin\signtool.exe" sign /f "XXXXX.p12" /p XXXXX /d "SpectralFiltre Installer" ".\installer\SpectralFiltre Installer.exe"

  REM -if %1 == 1 (
  REM -copy ".\installer\SpectralFiltre Installer.exe" ".\installer\SpectralFiltre Demo Installer.exe"
  REM -del ".\installer\SpectralFiltre Installer.exe"
  REM -)
